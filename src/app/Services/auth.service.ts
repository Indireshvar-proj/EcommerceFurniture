...
export class AuthService {
  ...
  Login(user : LoginDTO) : Observable<any> {
    let res = this.http.post<any>(`${environment.AuthAPIURL}/login`, user, {observe: 'response'}).pipe(
      tap(response => {
        // Save token to localStorage if it exists in response.body.token
        const token = response.body?.token;
        if (token) {
          localStorage.setItem('access-token', token);

          // Optionally, save expiration or user info if provided by backend
          // localStorage.setItem('expiration', response.body.expiration);
          // localStorage.setItem('user-data', JSON.stringify(response.body.user));
        }
      }),
      catchError(this.habdleError)
    );
  ...
  isAdmin() : Observable<any> {
    let headers : HttpHeaders = new HttpHeaders().set("Authorization", "Bearer " + localStorage.getItem("access-token"));
